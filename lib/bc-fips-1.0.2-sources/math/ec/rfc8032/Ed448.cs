...
namespace Org.BouncyCastle.Math.EC.Rfc8032
{
    internal abstract class Ed448
    {
        ...
        internal static IXof CreatePrehash()
        {
            return CreateXof();
        }
    ...
    //        x32 += (x31 >> 28); x31 &= M28UL;
...
#if false
        /// <remarks>Only for use by X448.</remarks>
        ...
        internal static void SignPrehash(byte[] sk, int skOff, byte[] ctx, IXof ph, byte[] sig, int sigOff)
        {
        ...
        internal static void SignPrehash(byte[] sk, int skOff, byte[] pk, int pkOff, byte[] ctx, IXof ph, byte[] sig, int sigOff)
        {
            byte[] m = new byte[PrehashSize];
            if (PrehashSize != ph.DoFinal(m, 0, PrehashSize))
                throw new ArgumentException("ph");
        ...
        internal static bool VerifyPrehash(byte[] sig, int sigOff, byte[] pk, int pkOff, byte[] ctx, IXof ph)
        {
            byte[] m = new byte[PrehashSize];
            if (PrehashSize != ph.DoFinal(m, 0, PrehashSize))
                throw new ArgumentException("ph");
