...
namespace Org.BouncyCastle.Math.EC
{
    ...
    {
        /*
         * Temporary code to provide randomness
         */
        private static SecureRandom m_testRandom;
        private static SecureRandom GetTestRandom()
        {
            lock (EMPTY_ZS)
            {
                if (m_testRandom == null)
                {
                    // check we are not still booting up.
                    if (!CryptoStatus.IsReady())
                    {
                        return CryptoServicesRegistrar.CreateService(FipsDrbg.Sha256)
                                    .FromEntropySource(new KATEntropyProvider())
                                    .Build(null, false, null);
                    }
                    else
                    {
                        byte[] nonce = Pack.UInt64_To_BE((ulong)DateTimeUtilities.CurrentUnixMs());
                        byte[] additionalInput = Strings.ToByteArray(typeof(ECPoint).FullName);

                        m_testRandom = CryptoServicesRegistrar.CreateService(FipsDrbg.Sha256)
                            .FromEntropySource(new SecureRandom(), false)
                            .Build(nonce, false, additionalInput);
                    }
                }
                return m_testRandom;
            }
        ...
        private class KATEntropyProvider: FixedEntropySourceProvider
        {
            internal KATEntropyProvider(): base(Hex.Decode(
