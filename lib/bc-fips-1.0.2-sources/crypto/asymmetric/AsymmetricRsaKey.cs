...
namespace Org.BouncyCastle.Crypto.Asymmetric
{
        ...
        : IAsymmetricKey
	{
        ...
        private static readonly IDictionary<BigInteger, WeakReference<KeyMarker>> markers =
            new Dictionary<BigInteger, WeakReference<KeyMarker>>();
        ...
        private static KeyMarker GetKeyMarker(BigInteger modulus)
        {
            ...
            {
                KeyMarker marker;
                if (markers.TryGetValue(modulus, out WeakReference<KeyMarker> existingRef))
                {
        ...
        private static void KeyMarkerFinalized(BigInteger modulus)
        {
            lock (markers)
            {
                if (markers.TryGetValue(modulus, out WeakReference<KeyMarker> existingRef))
                {
                    if (!existingRef.TryGetTarget(out KeyMarker _))
                    {
