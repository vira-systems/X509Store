...
namespace Org.BouncyCastle.Crypto.Asymmetric
{
	internal class KeyUtils
	{
        ...
        {
			BigInteger p = dhParams.P, q = dhParams.Q;
			if (q == null)
			{
		...
		internal static byte[] IsValidEdDSAPublicKey(byte[] keyData)
		{
			if (keyData.Length == Ed25519.PublicKeySize)
			{
				if (!Ed25519.ValidatePublicKeyFull(keyData, 0))
				{
					throw new ArgumentException("invalid Ed25519 public key data");
				}
			}
			else if (keyData.Length == Ed448.PublicKeySize)
			{
				if (!Ed448.ValidatePublicKeyFull(keyData, 0))
				{
					throw new ArgumentException("invalid Ed448 public key data");
				}
			}
			else
			{
				throw new ArgumentException("public key data wrong length");
			}
		...
		internal static byte[] GetEncodedPrivateKeyInfo(AlgorithmIdentifier algId, Asn1Encodable privKey, Asn1Set attributes, byte[] publicKey)
		{
			try
			{
				PrivateKeyInfo info = new PrivateKeyInfo(algId, privKey.ToAsn1Object(), attributes, publicKey);
