...
namespace Org.BouncyCastle.Crypto.Asymmetric
{
     ...
     */
		...
		private void checkKeyPairForConsistency(TPub publicKey, TPriv privateKey)
		{
            ...
            else if (publicKey is AsymmetricEdDsaKey && privateKey is AsymmetricEdDsaKey)
            {
                AsymmetricEdDsaPrivateKey priv = privateKey as AsymmetricEdDsaPrivateKey;
                AsymmetricEdDsaPublicKey pub = publicKey as AsymmetricEdDsaPublicKey;
                
                if (!Arrays.AreEqual(pub.GetPublicData(), priv.GetPublicData()))
                {
                    // FSM_TRANS:5.IKP.2, "IMPORTED KEY PAIR CONSISTENCY TEST", "USER COMMAND REJECTED", "Consistency test on imported key pair failed"
                    throw new ArgumentException("EdDSA public key not consistent with EdDSA private key");
                }
            }
