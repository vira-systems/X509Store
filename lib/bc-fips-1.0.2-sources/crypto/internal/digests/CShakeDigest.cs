...
namespace Org.BouncyCastle.Crypto.Internal.Digests
{
        ...
        : ShakeDigest
    {
            ...
            : base(bitLength)
        {
            if (bitLength == 128)
            {
                SelfTestExecutor.Validate(FipsShs.Algorithm.CSHAKE128, this, new FipsShs.XofKatTest(FipsKats.Values[FipsKats.Vec.CShake128]));
            }
            else
            {
                SelfTestExecutor.Validate(FipsShs.Algorithm.CSHAKE256, this, new FipsShs.XofKatTest(FipsKats.Values[FipsKats.Vec.CShake256]));
            }
