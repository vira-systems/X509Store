...
namespace Org.BouncyCastle.Crypto.General
{
	...
	public class Pbkd
    {
        ...
        public class OpenSslParameters : Parameters<GeneralAlgorithm>
        {
            ...
            internal OpenSslParameters(DigestAlgorithm digestAlgorithm, PasswordConverter converter, byte[] password, byte[] salt) : base(ALGORITHM_OPENSSL)
            {
                this.digestAlgorithm = digestAlgorithm;
                this.converter = converter;
                this.password = password;
            ...
            internal OpenSslParameters(DigestAlgorithm digestAlgorithm, PasswordConverter converter, char[] password) : this(digestAlgorithm, converter, converter.Convert(password), new byte[20])
            {
            }
        ...
        internal class OpenSslServiceImpl : IPasswordBasedDeriverBuilderService<OpenSslParameters>
        {
            ...
            {
                return new OpenSslDeriverBuilder(null, Arrays.Clone(password), null, new byte[20]);
            }

            public IPasswordBasedDeriverBuilder<OpenSslParameters> From(PasswordConverter converter, char[] password)
            {
                return new OpenSslDeriverBuilder(null, converter.Convert(password), converter, new byte[20]);
            }
        }
        ...
        internal class OpenSslDeriverBuilder : IPasswordBasedDeriverBuilder<OpenSslParameters>
        {
            ...
            internal OpenSslDeriverBuilder(DigestAlgorithm digestAlgorithm, byte[] password, PasswordConverter converter, byte[] salt)
            {
                this.digestAlgorithm = digestAlgorithm;
                this.converter = converter;
                this.password = password;
            ...
            public IPasswordBasedDeriver<OpenSslParameters> Build()
            {
                ...
                OpenSslParameters parameters = new OpenSslParameters(digestAlgorithm, converter, password, salt);

                OpenSslPbeParametersGenerator gen;
                if (digestAlgorithm == null)
                {
                    gen = new OpenSslPbeParametersGenerator();
                }
                else
                {
                    gen = new OpenSslPbeParametersGenerator(FipsShs.CreateDigest(digestAlgorithm));
                }
            ...
            {
                return new OpenSslDeriverBuilder(digestAlgorithm, password, converter, Arrays.Clone(salt));
            }
