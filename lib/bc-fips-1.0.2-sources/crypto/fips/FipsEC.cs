...
namespace Org.BouncyCastle.Crypto.Fips
{
	...
	public class FipsEC
	{
		...
		static FipsEC()
		{
			...
			// FSM_STATE:3.EC.0,"EC CVL Primitive 'Z' computation KAT", "The module is performing EC CVL Primitive 'Z' computation KAT verify KAT self-test"
			// FSM_TRANS:3.EC.0.0,"POWER ON SELF-TEST", "EC CVL Primitive 'Z' computation KAT", "Invoke EC CVL Primitive 'Z' computation KAT self-test"
			ecPrimitiveZTest();
			// FSM_TRANS:3.EC.0.1,"EC CVL Primitive 'Z' computation KAT", "POWER ON SELF-TEST", "EC CVL Primitive 'Z' computation KAT self-test successful completion"

			// FSM_STATE:3.EC.1,"EC CVL Primitive F2m 'Z' computation KAT", "The module is performing EC CVL Primitive F2m 'Z' computation KAT verify KAT self-test"
			// FSM_TRANS:3.EC.1.0,"POWER ON SELF-TEST", "EC CVL Primitive F2m 'Z' computation KAT", "Invoke EC CVL Primitive F2m 'Z' computation KAT self-test"
			ecPrimitiveF2mZTest();
			// FSM_TRANS:3.EC.1.1,"EC CVL Primitive F2m 'Z' computation KAT", "POWER ON SELF-TEST", "EC CVL Primitive F2m 'Z' computation KAT self-test successful completion"

			// FSM_STATE:3.EC.2,"ECDSA SIGN VERIFY KAT", "The module is performing ECDSA sign and verify KAT self-test"
			// FSM_TRANS:3.EC.2.0,"POWER ON SELF-TEST", "ECDSA SIGN VERIFY KAT", "Invoke ECDSA Sign/Verify  KAT self-test"
			ECDsaSigner signer = DSA_PROVIDER.CreateEngine(EngineUsage.GENERAL);
			// FSM_TRANS:3.EC.2.1,"ECDSA SIGN VERIFY KAT", "POWER ON SELF-TEST", "ECDSA Sign/Verify  KAT self-test successful completion"

			// FSM_STATE:3.EC.3,"ECDSA F2m SIGN VERIFY KAT", "The module is performing F2m ECDSA sign and verify KAT self-test"
			// FSM_TRANS:3.EC.3.0,"POWER ON SELF-TEST", "ECDSA F2m SIGN VERIFY KAT", "Invoke F2m ECDSA Sign/Verify  KAT self-test"
			doF2mDsaKatTest(signer);
			// FSM_TRANS:3.EC.3.1,"ECDSA F2m SIGN VERIFY KAT", "POWER ON SELF-TEST", "F2m ECDSA Sign/Verify  KAT self-test successful completion"
		}
		...
		public class KeyGenerationParameters : FipsParameters, IGenerationServiceType<KeyPairGenerator>, IGenerationService<KeyPairGenerator>
		{
			...
			public KeyGenerationParameters(IECDomainParametersID domainParametersID) : this(Alg, ECDomainParametersIndex.LookupDomainParameters(domainParametersID))
			{
			}
		...
		{
			private static readonly BigInteger r = new BigInteger(1, FipsKats.Values[FipsKats.Vec.ECStartupVecR]);
			private static readonly BigInteger s = new BigInteger(1, FipsKats.Values[FipsKats.Vec.ECStartupVecS]);
	    ...
	    private static void doF2mDsaKatTest(ECDsaSigner signer)
        {
			SelfTestExecutor.Validate(Alg, signer, new F2mDsaKatTest());
		}

		private class F2mDsaKatTest : VariantKatTest<ECDsaSigner>
		{
			private static readonly BigInteger r = new BigInteger(1, FipsKats.Values[FipsKats.Vec.F2mECStartupVecR]);
			private static readonly BigInteger s = new BigInteger(1, FipsKats.Values[FipsKats.Vec.F2mECStartupVecS]);
			...
			internal override void Evaluate(ECDsaSigner dsa)
			{
				...
				dsa.Init(false, kp.Public);
				if (!dsa.VerifySignature(FipsKats.Values[FipsKats.Vec.F2mECStartupVec], sig[0], sig[1]))
				{
		...
		private static void ecPrimitiveF2mZTest()
		{
			SelfTestExecutor.Validate(Alg, new PrimitiveF2mZTest());
		}

		private class PrimitiveF2mZTest : VariantInternalKatTest
		{
			internal PrimitiveF2mZTest() : base(Alg)
			{
			}
			internal override void Evaluate()
			{
				X9ECParameters p = NistNamedCurves.GetByName("B-233");
				EcDomainParameters parameters = new EcDomainParameters(p.Curve, p.G, p.N, p.H, p.GetSeed());
				BigInteger dValue = new BigInteger(1, FipsKats.Values[FipsKats.Vec.ECPrimitiveF2mStartupVec]);

				ECPoint Q = parameters.Curve.DecodePoint(Hex.Decode("03000518bce3b1b492c23094dcd7674c8ea6a3bcb7861bd2fb11be1999b796"));
		...
		private static AsymmetricCipherKeyPair getF2mKATKeyPair()
		{
			...
			// Verify the signature
			ECPublicKeyParameters pubKey = new ECPublicKeyParameters(
						parameters.Curve.DecodePoint(Hex.Decode("03000518bce3b1b492c23094dcd7674c8ea6a3bcb7861bd2fb11be1999b796")), // Q
						parameters);
