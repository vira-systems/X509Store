...
namespace Org.BouncyCastle.Crypto.Fips
{
    ...
    public class FipsRsa
    {
        ...
        {
            internal PssSignatureParameters(FipsAlgorithm algorithm, FipsDigestAlgorithm digestAlgorithm, FipsDigestAlgorithm mgfAlgorithm, int saltLength, byte[] salt) : base(algorithm, digestAlgorithm, mgfAlgorithm, saltLength, salt)
            {
            }
        ...
        internal static ICipherParameters GetSigKeyParams(IKey key)
        {
            if (key is AsymmetricRsaPublicKey)
            {
                ...
                if (CryptoServicesRegistrar.IsInApprovedOnlyMode())
                {
                    ...
                    if (bitLength < 2048 && bitLength != 1024 && bitLength != 1536) // includes 186-2 legacy sizes
                    {
                        throw new CryptoUnapprovedOperationError("Attempt to use RSA key with non-approved size: " + bitLength, key.Algorithm);
                    }
                }
        ...
        internal class PssSignerProvider : IEngineProvider<ISigner>
        {
            ...
            public ISigner CreateEngine(EngineUsage usage)
            {
                ISigner sig = new PssSigner(ENGINE_PROVIDER.CreateEngine(usage), FipsShs.CreateDigest(parameters.DigestAlgorithm), FipsShs.CreateDigest(parameters.MgfDigestAlgorithm), parameters.SaltLength, parameters.GetSalt());
