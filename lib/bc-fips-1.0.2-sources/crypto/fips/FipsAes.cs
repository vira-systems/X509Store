...
namespace Org.BouncyCastle.Crypto.Fips
{
    ...
    {
        //private static readonly string FPE_DISABLED = "Org.Bouncycastle.Fpe.Disable";
        //private static readonly string FF1_FPE_DISABLED = "Org.Bouncycastle.Fpe.Disable_Ff1";
        ...
        /// </summary>
        public static readonly FpeParameters Ff1 = new FpeParameters(new FipsAlgorithm(Alg, AlgorithmMode.FF1));

        /// <summary>
        /// AES as a FIPS SP800-38G format preserving encryption.
        /// </summary>
        public static readonly FpeParameters Ff3_1 = new FpeParameters(new FipsAlgorithm(Alg, AlgorithmMode.FF3_1));
        ...
        public class FpeParameters: Parameters
        {
            private readonly int radix;
            private readonly byte[] tweak;
            private readonly bool useInverse;

            internal FpeParameters(FipsAlgorithm algorithm): this(algorithm, 0, null, false)
            { 
            }

            private FpeParameters(FipsAlgorithm algorithm, int radix, byte[] tweak, bool useInverse): base(algorithm)
            {
                this.radix = radix;
                this.tweak = tweak;
            ...
            {
                return new FpeParameters(Algorithm, radix, this.tweak, this.useInverse);
            }
            ...
            public FpeParameters WithTweak(byte[] tweak)
            {
                if (tweak != null)
                {
                    return new FpeParameters(Algorithm, this.radix, Arrays.Clone(tweak), this.useInverse);
                }
                else
                {
                    return new FpeParameters(Algorithm, this.radix, null, this.useInverse);
                }
            }

            public FpeParameters WithUsingInverseFunction(bool useInverse)
            {
                return new FpeParameters(Algorithm, this.radix, this.tweak, useInverse);
            }
        }
    ...
    internal class Provider : IBlockEncryptorBuilderProvider<Parameters>, IBlockEncryptorBuilderProvider<ParametersWithIV>, IEncryptorBuilderProvider<ParametersWithIV>,
            ...
            IKeyWrapperProvider<WrapParameters>, IKeyUnwrapperProvider<WrapParameters>, IEncryptorBuilderProvider<FpeParameters>, IDecryptorBuilderProvider<FpeParameters>,
            IMacFactoryProvider<AuthenticationParameters>, IMacFactoryProvider<AuthenticationParametersWithIV>
        {
            ...
            private ICipherBuilder<FpeParameters> DoCreateCipherBuilder(bool forEncryption, FpeParameters parameters)
            {
                IBufferedCipher cipher = ProviderUtils.CreateBufferedCipher("FipsAES", parameters.Algorithm.Mode, parameters, forEncryption, aesEngineProvider);
                ICipherParameters prms = new Org.BouncyCastle.Crypto.Internal.Parameters.FpeParameters(null, parameters.Radix, parameters.GetTweak());
                cipher.Init(forEncryption, prms);
                return new CipherBuilderImpl<FpeParameters>(parameters, cipher);
            }
            ...
            public ICipherBuilder<FpeParameters> CreateEncryptorBuilder(FpeParameters algorithmDetails)
            {
                return DoCreateCipherBuilder(true, algorithmDetails);
            }

            public ICipherBuilder<FpeParameters> CreateDecryptorBuilder(FpeParameters algorithmDetails)
            {
                return DoCreateCipherBuilder(false, algorithmDetails);
            }
        }
            ...
            : IEngineProvider<Internal.IBlockCipher>
        {
            ...
            {
                Internal.IBlockCipher engine = SelfTestExecutor.Validate(Alg, new AesEngine(), new AesSelfTest());
                if (keyParameter != null)
                {
                    switch (usage)
                    {
                        case EngineUsage.ENCRYPTION_REVERSE:
                            engine.Init(true, new KeyParameter(Arrays.Reverse(keyParameter.GetKey())));
                            break;
                        case EngineUsage.DECRYPTION_REVERSE:
                            engine.Init(false, new KeyParameter(Arrays.Reverse(keyParameter.GetKey())));
                            break;
                        default:
        ...
        private class AesSelfTest
            : VariantKatTest<AesEngine>
        {
            internal override void Evaluate(AesEngine aesEngine)
            {
