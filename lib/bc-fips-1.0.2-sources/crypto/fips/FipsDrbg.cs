	...
	public class FipsDrbg
	{
		...
		private class DRBGHashSelfTest : DRBGSelfTest
		{
			...
			internal override void Evaluate()
			{
				...
				ISP80090Drbg d = new HashSP800Drbg(tv.Digest, tv.SecurityStrength, tv.EntropySource, personalisationString, nonce);

				{
					...
					{
						Fail("Self test " + algorithm.Name + ".1 failed, expected " + Strings.FromByteArray(Hex.Encode(expected)) + " got " + Strings.FromByteArray(Hex.Encode(output)));
					}
				}

				{
					byte[] expected = tv.GetExpectedValue(1);
					byte[] output = new byte[expected.Length];
					d.Generate(output, tv.GetAdditionalInput(1), tv.PredictionResistance);

					if (!Arrays.AreEqual(expected, output))
					{
						Fail("Self test " + algorithm.Name + ".2 failed, expected " + Strings.FromByteArray(Hex.Encode(expected)) + " got " + Strings.FromByteArray(Hex.Encode(output)));
					}
				}
			}
		}
		...
		private class DRBGHMACSelfTest : DRBGSelfTest
		{
			...
			internal override void Evaluate()
			{
				...
				ISP80090Drbg d = new HMacSP800Drbg(new HMac(tv.Digest), tv.SecurityStrength, tv.EntropySource, personalisationString, nonce);
		...
		private class DRBGCTRSelfTest : DRBGSelfTest
		{
			...
			internal override void Evaluate()
			{
				...
				ISP80090Drbg d = new CtrSP800Drbg(tv.Cipher, tv.KeySizeInBits, tv.SecurityStrength, tv.EntropySource, personalisationString, nonce);
		...
		private class DRBGTestVector
		{
			...
			private readonly Org.BouncyCastle.Crypto.Internal.IBlockCipher _cipher;
			private readonly int _keySizeInBits;
			private readonly IEntropySource _eSource;
			private readonly bool _pr;
			private readonly string _nonce;
			private string _personalisation;
			private readonly int _ss;
			private readonly byte[][] _ev;
			private readonly List<string> _ai = new List<string>();
			...
			public Org.BouncyCastle.Crypto.Internal.IBlockCipher Cipher
			{
				get { return _cipher; }
			}
			...
			public byte[] Nonce
			{
				get { return _nonce == null ? null : Hex.Decode(_nonce); }
			}

			public byte[] PersonalizationString
			{
				get { return _personalisation == null ? null : Hex.Decode(_personalisation); }
			}
